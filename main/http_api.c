...
static void append_ota_status(httpd_req_t *req)
{
    ...
    snprintf(buffer, sizeof(buffer),
             ...
             next_partition_address, next_partition_state,
             update_status);
...
static void append_reset_reason(httpd_req_t *req)
{
    ...
    snprintf(buffer, sizeof(buffer),
             ...
             "}",
             esp_reset_reason(),
             ESP_RESET_REASONS[esp_reset_reason()],
             ESP_RESET_DESCRIPTIONS[esp_reset_reason()]);
    httpd_resp_sendstr_chunk(req, buffer);
}
...
static void append_networking_to_output(httpd_req_t *req)
{
    ...
    snprintf(buffer, sizeof(buffer) - 1,
             ...
             "}",
             generated_hostname,
             ip, netmask, gw, wilma_current_ssid(),
             CONFIG_GDB_TCP_PORT,
             CONFIG_RTT_TCP_PORT, CONFIG_RTT_UDP_PORT,
             CONFIG_UART_TCP_PORT, CONFIG_UART_UDP_PORT);
    httpd_resp_sendstr_chunk(req, buffer);
}
...
{
    httpd_resp_sendstr(req, "{\"targets\":{\"current\":{\"name\":\"nrf52840\",\"ram\":262144,\"flash\":524188,\"cpu\":\"ARM Cortex-M4F\"},\"available\":[\"nrf52840\",\"mdf\"]}}");
    return ESP_OK;
}
